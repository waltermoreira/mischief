...
class Pipe(object):
    ...
    def _pack(self, ident, n_part, s):
        return struct.pack('c32si{}sc'.format(self.MAX_BUFSIZE),
                           '}', ident.hex, n_part, s, '\n')

    def _unpack(self, s):
        return struct.unpack('c32si{}sc'.format(self.MAX_BUFSIZE), s)
                             
    def _split_write(self, s):
        ident = uuid.uuid1()
        for i, part in enumerate(grouper(self.MAX_BUFSIZE, s)):
            self.fd.write(self._pack(ident, i, part))
        self.fd.write(self._pack(ident, -1, ''))
    ...
    def _split_read(self, s):
        _, ident, part, data, _ = self._unpack(s)
        if part < 0:
            # we have all the data, return it
            result = []
